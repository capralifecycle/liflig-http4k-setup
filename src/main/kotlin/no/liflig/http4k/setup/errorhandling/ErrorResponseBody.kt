 ...
 */
@Serializable
// We make this an open class instead of a data class, to allow extending the error response body
// with custom fields (some projects need this). We override toString, equals and hashCode below to
// mimic a data class.
open class ErrorResponseBody(
  ...
  companion object {
    val bodyLens =
        createJsonBodyLens(
            ...
            // Custom Content-Type specified by Problem Details specification:
  ...
  override fun toString(): String =
      "ErrorResponseBody(title='$title', detail=$detail, status=$status, instance='$instance', type=$type)"

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (other !is ErrorResponseBody) return false
